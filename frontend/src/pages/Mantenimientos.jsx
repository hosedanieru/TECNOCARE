...
export default function Mantenimientos() {
  ...
  const [form, setForm] = useState({
    equipo: '', tipo: 'PREVENTIVO', titulo: '',
    descripcion: '', fecha_programada: '', tecnico_asignado: '',
  })
  ...
  const cargarDatos = async () => {
    ...
    if (esTecnico) {
      params.tecnico_asignado = usuario.id
    }
  ...
  const handleSubmit = async (e) => {
    ...
    setForm({ equipo: '', tipo: 'PREVENTIVO', titulo: '', descripcion: '', fecha_programada: '', tecnico_asignado: '' })
    cargarDatos()
  }
    ...
    <div>
        ...
        <div>
          ...
          {esTecnico && (
            <p className="text-sm text-gray-500 mt-1">
              Mostrando solo los mantenimientos asignados a ti
            </p>
          )}
        </div>
        {puedeCrear && (
      ...
      {mostrarForm && (
        <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow mb-6 grid grid-cols-2 gap-4">
          ...
          <textarea placeholder="Descripción del problema o trabajo a realizar" required
      ...
      {mantenimientos.length === 0 ? (
        <div className="bg-gray-50 border border-gray-200 text-gray-500 px-4 py-8 rounded text-center">
          {esTecnico
            ? 'No tienes mantenimientos asignados actualmente.'
